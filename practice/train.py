...
class Trainer(object):
	def __init__(self, config):
		...
		self.experiment_name = config['experiment_desc'] + '_' + config['model']['g_name']
	...
	def img_from_tensor(self, image_tensor):
		np_img = image_tensor[0].cpu().float().numpy()
		np_img = (np.transpose(np_img, (1, 2, 0)) + 1) / 2.0 * 255.0
	...
	def _run_epoch(self, epoch):
		...
		for data in tq:
			...
			inp = self.img_from_tensor(inputs)
			gen = self.img_from_tensor(outputs.data)
			targs = self.img_from_tensor(targets.data)
			if i==0:
				self.metric_counter.add_image(np.hstack((inp, gen, targs)))
			i += 1
		tq.close()
